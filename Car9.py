...
flag = 0
pt = [[[(random.randint(0, 2)) for x in range(time_l)] for z in range(sep)] for y in range(pool_size)]
    ...
    for i in range(pool_size - survivors):
        ...
        for j in range(time_l):
            for k in range(sep):
                ...
                if m == mut:
                    new[k].append(random.randint(0, 2))
                else:
                    tpt = random.randint(0, 1)
                    new[k].append(pt[a][k][j]*(1-tpt) + pt[b][k][j]*tpt)
        pt.append(new)
    q += 1
    ttt = 0
    for i in range(sep):
